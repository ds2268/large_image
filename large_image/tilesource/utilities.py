...
def _computeFramesPerTexture(opts, numFrames, sizeX, sizeY):
    ...
    while True:
        ...
        fvert = int(max(math.ceil(f / math.floor(texSize / fw)), fvert))
        fh = int(math.floor(texSize / fvert / alignment) * alignment)
        if opts['maxFrameSize']:
            maxFrameSize = int(math.floor(opts['maxFrameSize'] / alignment) * alignment)
        ...
        fhorz = int(math.floor(texSize / fw))
        fvert = int(min(math.floor(texSize / fh), math.ceil(numFrames / fhorz)))
        fperframe = fhorz * fvert
        if textures > 1 and opts['frameGroup'] > 1:
            fperframe = int(math.floor(fperframe / opts.frameGroup) * opts.frameGroup)
        ...
        if textures > 1 and numFrames <= fperframe * (textures - 1):
            textures -= 1
            continue
        if fhorz >= 2 and math.ceil(f / math.floor(fhorz / 2)) * fh <= texSize / 2:
            texSize /= 2
            continue
...
def getTileFramesQuadInfo(metadata, options=None):
    ...
    :param options: dictionary of
        ...
        maxTextureSize: Limit the maximum texture size to a square of this
            size.  The size is also limited by the WebGL maximum size for
            webgl-based layers or 8192 for canvas-based layers.
        ...
        for fidx in range(f):
            quad = {
                ...
                'crop': {
                    ...
                    'y': sizeY,
                    'left': (fidx % ihorz) * fw,
                    'top': (ivert - int(math.floor(fidx / ihorz))) * fh - usedh,
                    'right': (fidx % ihorz) * fw + usedw,
                    'bottom': (ivert - int(math.floor(fidx / ihorz))) * fh,
                },
            }
